        ...
        for comp_type in comparisons:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    print(f"    ⚠ {comp_type} ODiff result: {result.stderr}")
                ...
                try:
                    ...
                    site_styles = await page.evaluate('''() => {
                        ...
                        const dropdowns = Array.from(document.querySelectorAll('nav .nav-drop')).map(drop => {
                            const pseudoStyles = window.getComputedStyle(drop, '::after');
                            return {
                                text: drop.textContent.trim(),
                                afterStyles: {
                                    marginLeft: pseudoStyles.marginLeft,
                                    borderTopColor: pseudoStyles.borderTopColor,
                                    borderWidth: pseudoStyles.borderWidth,
                                    position: pseudoStyles.position,
                                    top: pseudoStyles.top
                                }
                            };
                        });
...
"""
                ...
                orig_offset = float(orig_header.get('offsetFromCenter', 0))
                impl_offset = float(impl_header.get('offsetFromCenter', 0))
